...
const WECHAT_ID = 'AI-Sourcing-Alex';
        ...
        <div className="flex-grow text-center md:text-left border-t-2 md:border-t-0 md:border-l-2 border-slate-700/50 pt-8 md:pt-0 md:pl-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-6">
             ...
             <div className="text-center sm:text-left">
                 <p className="text-slate-400 mb-2">或手动添加，备注<strong className="text-emerald-400 font-bold mx-1">【工厂名】</strong>:</p>
                 <div className="relative flex items-center">
                     <input 
                         type="text" 
                         readOnly 
                         value={WECHAT_ID} 
                         className="bg-slate-800 border border-slate-600 rounded-lg pl-3 pr-16 py-3 w-56 text-white font-mono text-lg"
                     />
                     <button
                         onClick={handleCopy}
                         className="absolute right-1.5 top-1/2 -translate-y-1/2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-1.5 rounded-md flex items-center text-sm transition-all duration-200 disabled:bg-slate-600"
                         disabled={isCopied}
                     >
                         {isCopied ? <><Check className="w-4 h-4 mr-1"/> 已复制</> : <><Copy className="w-4 h-4 mr-1"/> 复制</>}
                     </button>
                 </div>
                 <p className="text-xs text-slate-500 mt-2">方便您在电脑端操作</p>
            </div>
          </div>
          <p className="text-xs text-slate-500 mt-6 text-center md:text-left">
